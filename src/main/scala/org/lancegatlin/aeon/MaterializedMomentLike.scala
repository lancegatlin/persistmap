...
trait MaterializedMomentLike[
  A,
  +B,
  +CRTP <: MaterializedMomentLike[A,B,CRTP]
] extends MaterializedMoment[A,B] with LocalMomentLike[A,B,CRTP] { self: CRTP =>
}
...
object MaterializedMoment {
  ...
  ) extends DelegatedLocalProjectionLike[A,B,MaterializedMomentImpl[A,B]] with
